...
export const displayAttributes = (template) =>
  template && Object.keys(template.attributes).filter( attribute_name =>
    template.attributes[attribute_name].shown && template.attributes[attribute_name].attribute_class != 'Magma::TableAttribute'
  );
...
const selectModel = (state, model_name) => selectModels(state)[model_name] || null;
...
const meetsTerms = (template, document,terms) => terms.every(
  term => matchesAttribute(term, template, document) || matchesAny(term, template, document)
)

const copyFrom = (documents) => (docs, name) => {
  if (name in documents) docs[name] = documents[name]; return docs;
}
...
const filterDocuments = (template, documents, filter) => {
  ...
  let filtered_documents = Object.keys(documents).filter(
    record_name => meetsTerms(template, documents[record_name], terms)
  ).reduce(
    copyFrom(documents), {}
  );
...
const matchesAny = (term, template, document) =>
  displayAttributes(template).some(
    att_name => {
      let txt = formatAttribute(document, template, att_name);
