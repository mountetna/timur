...
import {METIS_PATH_REGEX} from '../components/attributes/file_attribute';
...
export const filePathComponents = (metisPath) => {
  const results = metisPath.match(METIS_PATH_REGEX);
  const file_path_parts = results[3].split('/');
  let folder_name, file_name;
  if (file_path_parts.length === 1) {
    file_name = file_path_parts[0];
    folder_name = '';
  } else {
    file_name = file_path_parts[file_path_parts.length - 1];
    folder_name = file_path_parts[file_path_parts.length - 2];
  }

  return {
    bucket_name: results[2],
    file_name,
    folder_name
  };
};
