...
const seriesVars = (plot_series, vname) => plot_series.map(
  ( s, index) => `@${varName(`series${index}`,vname)}`
);
...
const seriesAddendum = (plot_series) => plot_series ? plot_series.map(
  ({variables}, index) => scriptBlock(`series${index}`, variables)
).join('\n') : '';
...
const bindSeriesData = ({name, variables}, varName, consignment) => ({
  name,
  variables: getConsignmentVars(varName, Object.keys(variables), consignment)
});
...
export const plotData = ({configuration: { plot_series }, plot_type}, consignment) => plot_series ? {
  plot_series: plot_series.map((series, index) => bindSeriesData(series, `series${index}`, consignment)),
...
export const plotWithScript = ([ plot, inputs ]) => ( { ...plot, plotScript: plotScript(plot, inputs) });
