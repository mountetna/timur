...
// or with no argument return the plot-specific variable names
const PLOTS = {
  lineplot: (plot_series) => {
    if (!plot_series) return [ 'xdomain', 'ydomain' ];
...
const plotAddendum = (plot_type, plot_series) => (plot_type && plot_series && (plot_type in PLOTS)) ?
  scriptBlock(plot_type, PLOTS[plot_type](plot_series))
  : '';
...
const bindPlotData = (plot_type, consignment) => (plot_type && consignment && (plot_type in PLOTS)) ?
  getConsignmentVars(plot_type, PLOTS[plot_type](), consignment) : {};
