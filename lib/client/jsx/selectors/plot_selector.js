...
const validSeries = (plot_type, series_type) =>
  validPlot(plot_type) && series_type &&
    series_type in PLOTS[plot_type].series_types;

const seriesTypeAddendum = (plot_type, series_type, index) =>
  !(validSeries(plot_type, series_type)
  && PLOTS[plot_type].series_types[series_type].computed) ? null :
    mapObject(
      PLOTS[plot_type].series_types[series_type].computed,
      (k,func) => func(index)
    );
  ...
  plot_series
    ? {
        plot_series: plot_series.map((series, index) =>
          bindSeriesData(series, `series${index}`, consignment)
        ),
