...
const seriesAddendum = plot_series =>
  plot_series
    ? plot_series
        .map(({ variables }, index) => scriptBlock(`series${index}`, variables))
        .join('\n')
    : '';

// plot-specific calculations to be added to the manifest
const plotAddendum = (plot_type, plot_series) =>
  plot_type && plot_series && plot_type in PLOTS
