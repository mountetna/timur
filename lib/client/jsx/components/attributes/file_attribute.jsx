...
const FileValue = ({ value }) =>
  ...
  ) : value.path instanceof File ? (
    <span className="file-upload">
      {' '}
      {value.path.name} ({value.path.type}){' '}
    </span>
        ...
        <input
          ...
          onChange={(e) =>
            reviseDocument(
              ...
              template,
              attribute,
              this.formatFileRevision(e.target.files[0])
            )
          }
        />
...
export default connect(null, { reviseDocument })(FileAttribute);
