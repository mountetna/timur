 ...
 * data.
 */
export const requestView = (model_nm, rec_nm, tab_nm, success, error, project_name = TIMUR_CONFIG.project_name)=>{
  return (dispatch)=>{
    ...
    var localSuccess = (response)=>{
      ...
      for(let tab_name in response.views[model_nm].tabs){
        let action = addTab(
          `${project_name}_${model_nm}`, // Model name is also the view name.
          tab_name,
