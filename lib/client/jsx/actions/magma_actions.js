...
import { startUploadActions } from 'etna-js/upload/actions/upload_actions';
...
export const sendRevisions = (model_name, revisions, success, error) => {
  return (dispatch, getState) => {
    let localSuccess = (response) => {
      // For multiple file revisions, might
      //   have a Promise chain with multiple responses.
    ...
    */
    ...
    _.forOwn(revisions, (revision, sampleId) => {
      _.forOwn(revision, (value, attribute) => {
        if (value.path && value.path instanceof File) {
          fileCache[sampleId] = {};
          fileCache[sampleId][attribute] = _.assign({}, value);
          revision[attribute] = _.assign(
            {},
            {
              path: '::temp'
            }
    ...
    postRevisions(formatRevisions(revisions, model_name), exchng)
      .then((response) => {
        ...
        let allPromises = [];
        _.forOwn(fileCache, (revision, sampleId) => {
          _.forOwn(revision, (value, attribute) => {
            if (value.path && value.path instanceof File) {
              // Get the Metis upload URL with HMAC signature
              const uploadUrl =
                response.models[model_name].documents[sampleId][attribute].path;
              allPromises.push(
      ...
      // TODO: We stil have to finalize the upload against Magma?
      .then(localSuccess)
      .catch(localError);
  };
};
