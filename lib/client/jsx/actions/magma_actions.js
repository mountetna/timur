...
const uploadTemporaryFiles = (model_name, revisions, response, dispatch) => {
  ...
  Object.keys(revisions).forEach((recordName) => {
    const revision = revisions[recordName];
    Object.keys(revision).forEach((revisionAttributeName) => {
      ...
      const tempRevision =
        response.models[model_name].documents[recordName][
      ...
      debugger;
      // TODO: Also need to send the metis_uid cookie value here,
      //      otherwise blobs will be rejected.
      dispatchUploadWork(
        dispatch,
        AddUploadCommand(
          Upload({
