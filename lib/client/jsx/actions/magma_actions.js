...
const uploadTemporaryFiles = (model_name, revisions, response, dispatch) => {
  ...
  const fileAttributeNames = attributeNamesByTypeForModel(modelAttributes, 'file').concat(
    attributeNamesByTypeForModel(modelAttributes, 'image')
  );
...
export const sendRevisions = (model_name, model_template, revisions, success, error) => {
  return (dispatch) => {
    ...
    postRevisions(formatRevisions(revisions, model_name, model_template), exchng)
      .then(localSuccess)
      .catch(localError);
  };
};
...
export const formatRevisions = (revisions, model_name, model_template) => {
  ...
  modelRevisions[model_name] = cleanFileCollectionRevisions(revisions, model_template);
...
const cleanFileCollectionRevisions = (revisions, model_template) => {
  ...
  const fileCollectionAttributeNames = attributeNamesByTypeForModel(
    modelAttributes, 'file_collection');
  ...
  Object.keys(revisions).forEach((record_name) => {
    ...
    Object.keys(revision).forEach((attribute_name) => {
      ...
      cleanRevisions[record_name][attribute_name] = revision[attribute_name].filter((rev) => {
        return "" !== rev;
      });
    });
  });
