...
export const createView = (view, exchange)=>{
    ...
    let exchangePromise = exchange.fetch(Routes.views_create_path(TIMUR_CONFIG.project_name), route_opts)
        .then(checkStatus)
        .then(parseJSON);
...
export const updateView = (view_obj, view_id, exchange)=>{
  ...
  let exchangePromise = exchange.fetch(Routes.update_view_json_path(TIMUR_CONFIG.project_name, view_id), route_opts)
    .then(checkStatus)
    .then(parseJSON);
