...
export const plotIndexUrl = (queryParams) => {
  let path = Routes.plots_path(CONFIG.project_name);
...
export const fetchPlots = ()=>{
  ...
  let route = Routes.plots_fetch_path(CONFIG.project_name);
...
export const createPlot = (plot)=>{
  ...
  let route = Routes.plots_create_path(CONFIG.project_name);
...
export const destroyPlot = (plot)=>{
  ...
  let route = Routes.plots_destroy_path(CONFIG.project_name, plot.id);
...
export const getPlot = (exchange) => (plot_id) => {
  return exchange.fetch(
    Routes.plots_get_path(CONFIG.project_name, plot_id)
  )
    .then(checkStatus)
    .then(parseJSON);
};
...
export const updatePlot = (plot)=>{
  ...
  let route = Routes.plots_update_path(CONFIG.project_name, plot.id);
