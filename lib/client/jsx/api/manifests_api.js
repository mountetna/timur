...
export const fetchManifests = (exchange)=>{
  ...
  let exchangePromise = exchange.fetch(Routes.fetch_manifests_path(CONFIG.project_name), route_opts)
    .then(checkStatus);
...
export const createManifest = (manifest, exchange)=>{
  ...
  let exchangePromise = exchange.fetch(Routes.create_manifest_path(CONFIG.project_name), route_opts)
    .then(checkStatus);
...
export const updateManifest = (manifestUpdates, manifest_id, exchange)=>{
  ...
  let exchangePromise = exchange.fetch(Routes.update_manifest_path(CONFIG.project_name, manifest_id), route_opts)
    .then(checkStatus);
...
export const destroyManifest = (manifest_id, exchange)=>{
  ...
  let exchangePromise = exchange.fetch(Routes.destroy_manifest_path(CONFIG.project_name, manifest_id), route_opts)
    .then(checkStatus);
